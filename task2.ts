...
const normalizeData = (unnormalizedData: IPost[]): NormalizedData => ({
	byId: unnormalizedData.reduce(function (acc, item) {
		return { ...acc, [item.id]: item }
	}, {}),
	allIds: unnormalizedData.reduce((acc, item) => {
		let res: any
		res = [...acc, item.id]
		return res
	}, []),
